...
DHT dht(DHTPin, DHTTYPE); // definimos pin y tio de sensor para libreria
...
void loop() //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
{
  if (millis() - tiempoUltimaLectura > 3000) // delay de encendido realizado con millis para no clavar el micro
  {
    ...
    diferencia_tension_carga_descarga = (voltaje_de_bateria - 46.4); // 46.4 voltaje de referencia tomado para bateria descargada y 50.8 para bateria cargada

    if (diferencia_tension_carga_descarga >= 0)
    {
      Porcentaje_carga = map(diferencia_tension_carga_descarga, 0.00, 4.40, 0, 100); // convierte el promedio de las tensiones medidas a porcentaje de carga bateria
    }
    else
    {
      Porcentaje_carga = 0;
  ...
  if (millis() - tiempoUltimaLectura_1 > retardo_1) // delay de encendido reaizado con millis para no clavar el micro
  {
    ...
    Serial.print("Porcentaje de carga bateria: ");
    Serial.print(Porcentaje_carga);
    Serial.println(" %\t");

    Serial.print("temperatura de bateria: ");
